...
`2nb ai status --json` is the fastest triage. It exposes `portability_status` (e.g. `ok`, `provider_unavailable`, `dimension_break`, `model_mismatch`, `mixed`, `unindexed`), the active `similarity_threshold` with its `similarity_threshold_source`, the embedding/embeddable/embedded doc counts, and a `providers[]` array carrying per-provider `reachable` / `disabled` / `reason`. There is no separate code field: the machine-readable cause is embedded in `reason` as a trailing `(code)`, e.g. `credentials were rejected (bad_credentials)`, and the same token appears in the degradation warning below. Read it before guessing why a search degraded.
...
| `"semantic search disabled: provider X not ready (CODE) — falling back to keyword search"` | The configured provider failed its readiness probe. `CODE` is one of `timeout`, `throttled`, `provider_unreachable`, `bad_credentials`, `access_denied`, `invalid_request`, `unknown` | Branch on the code: `timeout`/`throttled`/`provider_unreachable` usually clear on their own, `bad_credentials`/`access_denied` need `2nb ai status`, `unknown` means the probe failed unrecognizably. BM25 still works — results still return, just without vector ranking. |
