...
| `2nb move <src> <dst> [--dry-run] [--force]` | Move/rename a note to a new vault-relative path AND rewrite every `[[wikilink]]` across the vault that points at it (preserving `#heading`/`#^block`/`\|alias`/`!`-embed suffixes and the bare-vs-path form; links inside code are never touched). The strongest write surface: it edits OTHER notes. **Always `--dry-run` first** to preview the rename, per-note rewrites, and the ambiguous links it would skip. Without `--force`, a move is refused when a bare `[[name]]` link is ambiguous (the name matches more than one note); `--force` then rewrites only the unambiguous path-qualified links. The target file is moved LAST (after referencing notes), so a crash leaves links pointing at the still-present old name. JSON: `{moved, rewritten, skipped_ambiguous, failed}`. |
