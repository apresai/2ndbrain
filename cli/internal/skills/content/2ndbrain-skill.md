...
- Genuinely new: `kb_create` / `2nb create --type <type> --title "..."` mints the UUID and type-appropriate frontmatter. Then write the body and add `[[wikilinks]]` to the docs search surfaced. To file the doc under a subdirectory, pass `--path <subdir>` (CLI) or the `path` argument (`kb_create`); the directory is created if missing, otherwise the doc lands in the vault root. Titles allow only letters, numbers, spaces, and basic punctuation (no em-dashes or slashes) since the title becomes the filename slug. **Prefer `2nb create` / `kb_create` over hand-writing a `.md` file:** it slugifies the filename (lowercased, spaces to dashes, other punctuation dropped), mints the UUID, and sets the `title` frontmatter, so the note is findable and index-consistent. A raw filesystem write reintroduces spaces/special characters and is not indexed until the next `2nb index`.
...
2nb move old.md archive/old.md              # apply (refused if a bare [[name]] is ambiguous, unless --force)
                                            # "name" is anything the resolver accepts: filename, path suffix, title, alias
...
| `obsidian migrate-properties` | `--write` (previews without it). Rewrites quoted `created`/`modified` (and any schema `date`/`datetime` field) into the plain form Obsidian types as **Date and time**; date-shaped properties YOU author are reported, never touched. Idempotent; snapshots each note so `2nb polish <path> --undo` reverses it |
| `obsidian register-types` | `--write` `--force` (previews without `--write`). Merges 2nb's property types into `.obsidian/types.json`. The only Obsidian setting 2nb writes; refused while Obsidian has the vault open, and refused until `migrate-properties` has run |
...
All commands support `--json`, `--yaml`, `--csv`, `--format` (also `tsv`/`raw`/`md`/`text`; listings add `paths`/`tree` and `--total`), `--porcelain`, `--vault <path>`, `--copy`, and `--verbose`. Prefer `--json` in scripts and agent pipelines. An explicit `--format` is either honored or refused by name, never silently ignored: `raw`/`md` emit a document body and are REFUSED on a value that has none (a search result list, a report), including by `search` and `list` when nothing matched; in `csv`/`tsv` a composite cell such as `frontmatter` is compact JSON; the JSON-event-stream commands (`models bench`, `ai engine pull`/`rm`) accept only `--json`; `text` is a READING format (a row list renders as `name=value` pairs per line, a single struct as `name: value` lines) and is not quoted or escaped, so parse `tsv` or `json` instead; an empty LISTING is `[]` under `--json` and `--yaml` and a header row with no data under `csv`/`tsv`, while `search` and `ask` are the exception (they carry an envelope object, not a bare array, so a zero-hit `search` emits its full `{mode, warnings, results}` envelope under `--json` and NOTHING on stdout under `yaml`/`csv`/`tsv`/`text`, and `ask` exits non-zero when retrieval finds nothing); and `git diff`, `export-context` and `skills show` are the mirror image, since their output IS a body: `raw`/`md`/`text` give you it, `--json` wraps it in a record (`{path, diff}` / `{bundle, docs, chars}` / `{slug, agent, project_path, user_path, version, content, chars}`), and `csv`/`tsv`/`yaml` are refused because none of them is a row set. A command that WRITES emits a small result record naming what changed (`models add|remove|enable|disable|enable-state`, `metrics clear`, `config set`, `vault create`/`set`, `skills install`/`uninstall`, `mcp install`/`uninstall`/`reap`), so `--json` on any of them is parseable rather than empty or prose. In `csv`/`tsv` a nil pointer is an EMPTY cell, never the Go token `<nil>`, and a single scalar payload is one plain cell rather than a quoted JSON string. **You should not need to shell out to `--help` to use these commands.** The "[Copy-paste recipes](#copy-paste-recipes)" section gives the exact invocation for every common task, "[Flags by command](#flags-by-command)" lists the flags each command takes, and "[Worked JSON examples](#worked-json-examples)" show the output shapes. (`2nb <command> --help` and `2nb --help` still exist to confirm a detail, but build the command from the recipes first rather than invoking it, reading the help, and retrying.)
...
| `2nb graph <path>` | The document's direct inbound and outbound links as a JSON adjacency list (ONE hop; use `2nb related <path> --depth N` to walk further). Takes a path, like `related`/`backlinks`/`links`; there is no whole-vault form |
...
`2nb` writes only the gitignored `.2ndbrain/` sidecar and never rewrites a note's body on its own. One further exception, outside the sidecar and outside your notes: `2nb obsidian register-types --write` merges `.obsidian/types.json`, and nothing else under `.obsidian/` is ever written except the plugin bundle from `2nb plugin install`. The body-write commands below (`append`, `prepend`, `replace`, and `polish --write`) are the explicit, opt-in exceptions: they rewrite the body in place only when you invoke them. `meta --set` and `tags rename` rewrite the frontmatter in place. Everything else here either creates or deletes whole files, or returns text for you to apply yourself.

| Command | Purpose |
|---------|---------|
| `2nb create --type <type> --title "Title" [--path <subdir>] [--content <body>]` | Create document from template. Generates UUID, timestamps, and type-appropriate frontmatter. `--path` files it under a vault-relative subdirectory (created if missing; default vault root). `--content "$(cat file.md)"` sets the body in one shot instead of the template. On a same-title collision: default makes a `<slug>-1.md`; `--overwrite` replaces in place (reuses the id); `--append` appends to the existing note; `--allow-duplicate` skips the content-hash dedupe guard. Human output echoes the resulting slug filename and the title (`Created note: my-note.md (title: "My Note")`); `--json` returns `path` + `title`. |
...
| `2nb obsidian migrate-properties [--write]` | One-shot repair for notes written before 0.23.0: rewrites quoted date values into the plain form Obsidian types as **Date and time**. Previews by default; `--write` snapshots each note first. Touches only `created`, `modified` and schema-declared `date`/`datetime` fields, leaves every other byte of a note alone, and is idempotent. |
| `2nb obsidian register-types [--write]` | Declares those property types in `.obsidian/types.json` (merge-only, backup-first, refused while Obsidian has the vault open and until the migration has run). |
| `2nb meta <path> --set key=value` | Update one or more frontmatter fields in place, with schema + status-transition validation. A value in a date field (`created`, `modified`, or any schema `date`/`datetime` field) is stored in the plain form Obsidian types as a date, so an edit never reverts a note to Text. Rewrites the file's YAML frontmatter; the body is preserved. Array fields (`tags`, `aliases`, schema `list`/`tags` fields) are coerced to a YAML list, comma-split, replace semantics (`--set tags=a,b`); use `tag add`/`tag remove` for incremental tag edits. |
...
| `2nb delete <path> [--force]` | Delete from disk and index. Prompts for confirmation interactively; **pass `--force` for non-interactive/agent use** (a non-terminal stdin waits 60s, then reports the note was NOT removed and points at `--force`, so an agent never hangs). |
| `2nb move <src> <dst> [--dry-run] [--force]` | Move/rename a note to a new vault-relative path AND rewrite every `[[wikilink]]` AND markdown-style `[text](path.md)` link across the vault that points at it (wikilinks keep `#heading`/`#^block`/`\|alias`/`!`-embed suffixes; markdown links keep the `[label]` text, any `#anchor`/`?query` suffix, and the `.md` extension; both keep the bare-vs-path form. External-URL and anchor-only markdown links are skipped; links inside code are never touched). The strongest write surface: it edits OTHER notes. **Always `--dry-run` first** to preview the rename, per-note rewrites, and the ambiguous links it would skip. Without `--force`, a move is refused when a bare `[[name]]` link is ambiguous (the name matches more than one note), where `name` is anything the resolver accepts: a filename, a path suffix, a frontmatter title, or an alias, so two notes sharing a TITLE block a link to that title too. `--force` then rewrites only the unambiguous path-qualified links. The target file is moved LAST (after referencing notes), so a crash leaves links pointing at the still-present old name. JSON: `{moved, rewritten, skipped_ambiguous, failed}`. |
...
| `2nb export-context --types <types>` | Generate a CLAUDE.md-compatible context bundle. The bundle is a document body: the default form and `--format raw`/`md`/`text` emit it, `--json` wraps it as `{bundle, docs, chars}`, and `csv`/`tsv`/`yaml` are refused |
...
| `2nb config bedrock` | Show or set machine-local Bedrock region + token (`~/.config/2nb/bedrock.json`). No vault required. `--json` redacts the token |
| `2nb config set-key <provider>` | Store a provider API key (bedrock writes the machine file; OpenRouter stays Keychain-only) |
...
| `2nb eval` / `eval tune` / `eval answers` | Retrieval scorecard (Recall@10 / R@1 / MRR over a cached QA set generated from the vault; cost-gated one-time generation), a threshold/weight sweep that prints suggested `config set` commands (suggest-only, near-zero cost), and an LLM-jury answer scorecard (self-judged by default, `--judges` for a panel, cost-gated) |
| `2nb models list` / `models test <id>` / `models verify [--yes]` / `models bench` / `models wizard [--set-active]` | Verified catalog, smoke test, batch account-access probe (records pass/fail per model with a classified code; pass `--yes` non-interactively), benchmark favorites, end-to-end discover→test→save wizard (`--set-active` also writes the chosen models into the vault config) |
| `2nb models policy set --provider bedrock --enable-only anthropic,deepseek` / `models policy show` / `models policy clear --provider <p>` | Persistent enable-only vendor policy per provider: models from non-chosen vendors are disabled in selection dropdowns, including FUTURE discoveries (they arrive pre-disabled). Explicit per-model `models enable/disable` still wins; the active models are never policy-disabled. `--scope vault\|global` (vault overrides global), `--dry-run` previews the per-vendor effect; bare `models policy` = `show` |
...
`2nb ai status --json` is the fastest triage. It exposes `portability_status` (e.g. `ok`, `provider_unavailable`, `dimension_break`, `model_mismatch`, `mixed`, `unindexed`), the active `similarity_threshold` with its `similarity_threshold_source`, the embedding/embeddable/embedded doc counts, and a `providers[]` array carrying per-provider `reachable` / `disabled` / `reason`. There is no separate code field. When a provider can classify its own failure (today: bedrock, after a probe) `reason` ends in the machine-readable cause in parentheses, e.g. `credentials were rejected (bad_credentials)`, the same token the degradation warning below carries. Other rows carry a plain-language reason with no code (`disabled in vault config`, `no API key (set $OPENROUTER_API_KEY …)`, `not running on http://localhost:11434`), so treat the trailing code as present-if-classified, not guaranteed. Do NOT just extract the last parenthesis: even bedrock says `region not set (ai.bedrock.region)` and `bedrock not registered (check ai.provider setup)` when it never got as far as probing. Match the parenthesized value against the known codes below and ignore it otherwise. Read it before guessing why a search degraded.
...
| `"semantic search disabled: provider X not ready (CODE) — falling back to keyword search"` | The configured provider failed its readiness probe. `CODE` is one of `timeout`, `throttled`, `provider_unreachable`, `bad_credentials`, `access_denied`, `not_found`, `invalid_request`, `unknown`; treat an unrecognized code as `unknown` rather than assuming the set is closed | Branch on the code: `timeout`/`throttled`/`provider_unreachable` usually clear on their own, `bad_credentials`/`access_denied` need `2nb ai status`, `unknown` means the probe failed unrecognizably. BM25 still works: results still return, just without vector ranking. |
