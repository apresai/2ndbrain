...
| AI Hub | AIHubView.swift | Single merged surface (AI menu > AI… · Cmd+Shift+,) for everything AI. Three sections: Providers (enable/disable Bedrock / OpenRouter / Ollama cards with live status), Active (current embedding + generation with inline swap), Catalog (grouped by vendor within type: Embedding Models first, then Generation Models; each group is a collapsible disclosure with count, latest-first model rows, and equal-weight `Enable all` / `Disable all` bulk buttons). Search field filters by model ID or vendor name. Per-row actions: Test · Set active · Enable/Disable · Discover. Replaces AI Setup Wizard, Test AI Connection, and Model Wizard. Observes `modelsCatalogVersion` so CLI edits from another terminal refresh live. Vendor grouping uses `VendorInfo.from(modelID:provider:)` mirroring Go's `ai.VendorOf`. Provider disable is vault config: `ai.<provider>.disabled` — hides every model from that provider until re-enabled |
