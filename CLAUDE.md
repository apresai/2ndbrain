...
| `internal/instructions` | Managed "2ndbrain" reference block for an agent's global memory file (`~/.claude/CLAUDE.md`); sentinel-delimited, version/sha-stamped, backup-safe writer (install/configured/uninstall) |
...
- **Catalog** — grouped by vendor within type (Embedding, Generation, then **Reranking**); each group is a collapsible disclosure with count, latest-first rows, and `Enable all` / `Disable all` bulk buttons. The Home tab's AI card also shows a Rerank row (Off by default).
