...
| `ai status` | | Show AI provider, models, readiness, embedding count, and vault portability state (dimension mismatch, model mismatch, provider unavailable, etc.) with one-line fix hints; pricing shows "unknown" when no pricing data is available, "free" only for explicitly-confirmed free models |
...
| `models test` | `<model-id>`, `--provider`, `--type`, `--save`, `--scope` | Smoke-test a model (embed or generate probe). `--save` writes the result to the user catalog regardless of pass/fail — successes set `tier=user_verified` + `tested_at` + `test_latency_ms`; failures set `test_error` + `tested_at` so the picker can surface them. Default `--scope` is `vault` |
| `models add` | `<model-id>`, `--provider`, `--type`, `--scope`, `--name`, `--dimensions`, `--context-length`, `--price-in`, `--price-out`, `--price-request`, `--similarity-threshold`, `--notes` | Add or update a model in the user catalog (per-vault `.2ndbrain/models.yaml` by default, or global `~/.config/2nb/models.yaml` with `--scope global`). When the entry already exists, `models add` *merges*: `Enabled`, `TestedAt`, `TestLatencyMs`, `Benchmark` are preserved unless explicitly re-set; only flags marked `Changed` by Cobra take effect, so a `--similarity-threshold`-only call doesn't flip `PriceSource` to `user`. `--similarity-threshold` is embedding-only and persists into the threshold resolution chain; `--price-request` is for per-request priced models (e.g. TwelveLabs Marengo embed) |
| `models remove` | `<model-id>`, `--provider`, `--scope` | Remove a model from the user catalog |
| `models enable` | `[<model-id>]`, `--provider`, `--scope`, `--vendor` | Mark a model enabled. With `--vendor <name>` (e.g. `anthropic`, `amazon`, `google`) applies to every model from that vendor in one write — the GUI's per-group bulk toggle. `--vendor` and `<model-id>` are mutually exclusive |
| `models disable` | `[<model-id>]`, `--provider`, `--scope`, `--vendor` | Hide a model (or every model from a vendor with `--vendor`) from selection dropdowns. Still listed by bare `2nb models list` |
| `models enable-state` | `<model-id>`, `--provider`, `--scope`, `--state default\|enabled\|disabled` | Set the tri-state `Enabled` pointer in one shot. `default` clears the field so tier defaults take over; `enabled`/`disabled` correspond to the `models enable` / `models disable` shortcuts. Used by the GUI picker's Enable State menu |
...
| `models bench` | `--model`, `--probe embed\|generate\|retrieval\|search\|rag`, `--provider`, `--summary-scope global\|vault` | Benchmark models against the vault. The `retrieval` probe is zero-API: it scores stored embeddings against resolved wikilinks (MRR/Recall@K) without calling any provider. Run history is always written to `.2ndbrain/bench.db`. The per-model summary (avg latency + quality score) is written to the user catalog at `--summary-scope` (default `global` so a shared/git-tracked vault doesn't churn `.2ndbrain/models.yaml`; pass `vault` to keep history per-vault). When `--json` (or `--format json`), emits line-delimited events: `model_start`, `probe_start`, `probe_result`, `summary`, `done` |
...
| `completion install` | `--dir <path>` | Install zsh completion to the existing completion dir from `.zshrc` (or `~/.zsh/completions/_2nb`, or `--dir`); idempotently updates `~/.zshrc` with fpath + compinit block placed before early-return guards and before any existing compinit/fpath setup (gcloud, Homebrew, etc.); compinit runs unconditionally so it works even when another tool initializes the completion system first; warns with version info if multiple `2nb` binaries are found on PATH |
...
**Similarity threshold:** hybrid search drops vector hits whose cosine similarity is below the active threshold. Resolution chain (in `AIConfig.ResolveSimilarityThresholdFull`): vault `ai.similarity_threshold` (if > 0) → user catalog `RecommendedSimilarityThreshold` from `~/.config/2nb/models.yaml` or `.2ndbrain/models.yaml` → the active model's `RecommendedSimilarityThreshold` from `BuiltinCatalog()` → `DefaultSimilarityThreshold` (`0.20`). Different embedding models have very different baseline similarity distributions. Builtin recommendations: Nova-2 `0.65` (measured on a real vault), Nemotron-VL `0.60`, nomic-embed-text/Titan-v2/Cohere-embed `0.50`, mxbai/snowflake/bge-m3 `0.55`, all-minilm `0.35` — the rest are estimates from each model's training objective. Configure per-vault via `2nb config set ai.similarity_threshold 0.65`, save per-vault or global calibration via `2nb models calibrate --save`, or override per-query with `2nb search "foo" --threshold 0.35`. `2nb ai status` prints the active value and source (`vault config` / `user calibration` / `model recommendation` / `default`). `2nb models list` shows the active recommendation per model in a THRESHOLD column. Results display `(rrf=X.XXX, cos=Y.YYY)` so you can judge semantic relevance directly — the RRF score alone is opaque.

**Calibration:** `2nb models calibrate [--samples N] [--save] [--scope vault|global] [--seed N]` samples random doc pairs from the active vault, computes cosine distribution (p50/p90/p95/p99), and recommends a threshold at `p95 + 0.01` rounded up. Samples per-doc embeddings (one per markdown file) via `store.DB.AllEmbeddings`. `--save` upserts a user-catalog entry (`scope=vault` default) carrying only the threshold; existing fields on a matching builtin entry are preserved through `mergeFields`. Default sample count is 500; small vaults clamp to `n*(n-1)/2` unique pairs.
...
| AI Hub | AIHubView.swift | Single merged surface (AI menu > AI… · Cmd+Shift+,) for everything AI. Three sections: Providers (enable/disable Bedrock / OpenRouter / Ollama cards with live status), Active (current embedding + generation with `Change` button per slot), Catalog (grouped by vendor within type: Embedding Models first, then Generation Models; each group is a collapsible disclosure with count, latest-first model rows, and equal-weight `Enable all` / `Disable all` bulk buttons). Per-row action is now `Details`, which opens the `ModelCatalogPickerView` modal (sidebar + detail; filters: type/provider/tier/enabled/tested/compatible; sort modes: Best/Cheapest/Fastest/Newest/Name; per-model actions: Test, Set Active, Set Active + Re-embed, Enable State tri-state, Similarity Threshold override, Cost Preview per probe kind, Benchmark with streaming events). Replaces AI Setup Wizard, Test AI Connection, and Model Wizard. Observes `modelsCatalogVersion` so CLI edits from another terminal refresh live. Vendor identity (vendor / vendor_display / family / version_sort_key) and `compatible` flag are computed by the Go CLI in `applyCatalogUIFields` and sent over JSON — Swift no longer mirrors that logic. Provider disable is vault config: `ai.<provider>.disabled` — hides every model from that provider until re-enabled. `Set Active` is gated on `appState.isIndexing` and refused at the AppState layer to prevent mixed-model embeddings during a rebuild |
