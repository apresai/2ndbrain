...
**An UNQUOTED frontmatter scalar is not a Go string, and every read of one goes through `document.ScalarText`** (`document/frontmatter_scalar.go`). yaml.v3 decodes into `any` with the full core schema, so `modified: 2020-01-01T00:00:00Z` is a `time.Time`, `status: true` a `bool`, `title: 12345` an `int`: a bare `meta[key].(string)` assertion silently fails for all of them and leaves the field EMPTY. That is the shape Obsidian's own Date property writes, while `2nb create` and `2nb meta` quote theirs, so it held for 2nb-authored notes and failed for hand-edited and imported ones with no warning anywhere: `documents.modified_at` stayed empty and `stale` (which filters `modified_at != ''`) omitted the note. `frontmatterTime` (created, modified, any future date field) accepts a string or a `time.Time` and normalizes to RFC3339, the format the column stores and the format `stale` and `list --sort modified` parse back; it refuses a number or a boolean, which are not dates. `frontmatterText` (id, title, type, status) and the tag/alias list readers coerce any scalar. **Never normalize the parsed frontmatter MAP.** `Serialize` re-marshals every key of `Frontmatter` through `UpdateDocumentFrontmatterAST`, so writing a string back over a `time.Time` would make an unrelated `meta --set`, `tag add` or `polish --write` requote a date line the user never touched, against the write-surface guarantee; only the struct fields the index reads are normalized, and `TestContract_MetaSetLeavesAnUnquotedDateAlone` pins that. `meta --get`'s default branch renders through the same helper, since `--json` and `--format text` already emitted RFC3339 and only the plain one printed Go's `2020-01-01 00:00:00 +0000 UTC`.
