...
| `ai engine` | Manage the bundled llama.cpp engine (the `llama-local` provider): `pull <id>...` downloads + sha256-verifies GGUF weights into `~/Library/Caches/2nb/models/` (progress bar; `--json` emits line-delimited `{model,status,done,total}` progress for the GUI; a 60s idle watchdog aborts a stalled transfer), `serve` runs `llama-server` for the configured roles, `install`/`bootout` wire/remove the launchd agent. Model pins + licenses live in `internal/llama/models.go` |
...
- **Local models** — a **"Download local models"** button (NSAlert confirm → `2nb ai engine pull --json` streamed via a `Process` `readabilityHandler` into an observable `LocalModelDownload` progress bar) that fetches the ~4 GB bundled stack (EmbeddingGemma + Gemma 4 E2B + bge reranker); `PullEventStream` line-buffers the JSONL.
