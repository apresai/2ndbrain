...
- **A provider retry is never silent.** All four Bedrock retry loops (classic InvokeModel and Converse, Cohere rerank, the mantle 429 loop) log one INFO line and record into the `ai.RetryCounter` on the call's context; `metrics.db.embed_retries` (schema v3) keeps the count per operation, and `vault status` / `ai status` advise lowering `ai.embed_concurrency` only when the last build retried AND the configured value exceeds the automatic one. The CLI's `slowCallNotice` prints one self-erasing stderr line past 2s under noteWrite's rule (TTY, not `--porcelain`), and it REUSES a counter already on the context rather than shadowing it. Never wire the notice into the bulk embed pass: its per-document progress printer owns the same terminal line.
...
**A note that will not parse is data to report, not a run to fail.** The whole-vault index skips it, names it (`index --json` `unparseable[]`, one stderr line per file from `--force-reembed`, the MCP `kb_index` result, one `N file(s) could not be parsed` WARN per run with the paths at INFO), and DROPS its index row the way a deleted file's row is dropped, so the last readable version stops answering searches. A parse failure is never counted as a failed EMBEDDING (nothing was sent), which is what let one bad note fail a whole `--force-reembed` and roll back every good embedding. `index --doc` on such a note still exits non-zero. Obsidian's own template folders (`.obsidian/templates.json` `folder`, Templater's `templates_folder`) are excluded from both the walk and `index --doc`, matched on whole path segments, and named by `vault status`.
