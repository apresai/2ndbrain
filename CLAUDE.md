...
| `graph <path>` | Link graph for one document as a JSON adjacency list |
...
- **A builtin fact is the builtin's unless the user stamped it.** The user catalog owns a row's Name, Dimensions and ContextLen only when `fact_source: user` (`ai.FactSourceUser`) says the user typed them, which only `models add --name/--dimensions/--context-length` writes; ConfigHint and Recommended are builtin-owned outright. Every save path seeds its row from the MERGED catalog, so without the stamp a copy of a builtin fact was indistinguishable from an authored one and then shadowed the builtin's own later correction: a stale `context_length` kept `models list` wrong and, through `inheritModelFacts`, spread to freshly discovered per-region rows. Read side: `mergeFields(base, top, baseIsBuiltin)` (the global-under-vault user overlay passes false, since neither user file owns anything). Write side: `preserveUserFacts` carries stamped facts, a price override, notes, and the route's benchmark from the RAW stored row, and drops an unstamped fact for a builtin model (the self-heal) while keeping one for a model no builtin declares. Never seed a persisted row from the merged catalog.
