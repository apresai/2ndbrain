...
**An explicit `--format` is always honored, never ignored.** A report command routes every non-empty format through `emitStructured` (`root.go`), so csv/tsv/yaml render and raw/md are refused when the value has no document body; checking only for `--json` and falling through to a human printer is the anti-pattern that made a dozen commands print prose for csv and for raw. `search`/`list` refuse raw/md ahead of their zero-result early return, so the refusal depends on the command, not the row count. A JSON-event-STREAM command (`models bench`, `ai engine pull|rm`) cannot render as one document and refuses other formats by name (`refuseNonJSONStream`). `git diff` is the exception, and it is symmetric: its output IS a body, so raw/md/text emit the diff, json wraps it, and csv/tsv/yaml are REFUSED (a diff is not a row set). In csv/tsv, composite cells are compact JSON (`output.delimitedCell`), never Go syntax.
...
- **A user-catalog threshold is a calibration only if the user authored it.** `models calibrate --save` and `models add --similarity-threshold` are the only writers and both stamp `threshold_source: user` (`ai.ThresholdSourceUser`); every probe/benchmark/promotion save goes through `preserveUserThreshold`, which strips the field and restores only a stored user value. `ai.IsUserThreshold` is the single read-side predicate: stamped is the user's, unstamped-and-equal-to-the-builtin is a mirror and ignored, unstamped-and-different is a pre-stamp calibration and kept. Never seed a persisted row's threshold from the merged catalog; that is what made one `models bench` run (default `--summary-scope global`) report a builtin value as a calibration in every vault on the machine.
