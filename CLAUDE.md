...
An optional cross-encoder rerank stage reorders the hybrid candidate pool by relevance before the caller's limit trims it. It lives in the shared `internal/retrieve` pipeline (so `search`/`ask`/MCP all get it), reads raw chunk TEXT (decoupled from the embedder), and degrades to the RRF order on any failure. Provider: **Cohere Rerank 3.5 on Amazon Bedrock** (`bedrockagentruntime.Rerank`, model `cohere.rerank-v3-5:0`, **us-east-1 in-region only** — colocated with Nova-2), same AWS auth as the embedder (`ai.BedrockReranker`). A local reranker isn't viable in the pure-Go build (ONNX needs CGO; Ollama has no rerank endpoint). Config: `ai.rerank.enabled` (default **false**), `ai.rerank.model`, `ai.rerank.candidate_docs` (over-fetch pool, default 50, ≤100 per the Bedrock per-query cap). `ai status` shows the slot; `retrieve.New` resolves the reranker from the registry only when enabled, so off = zero cloud calls.
