...
| `skills list\|install\|uninstall\|show` / `skills doctor` | SKILL.md management per agent (version-stamped; `skills list` self-heals stale unmodified installs; `install`/`uninstall` emit one result row per agent, `show` is a BODY like `git diff`) / verify the skill + that `2nb` resolves on PATH |
...
**An explicit `--format` is honored or refused by name, never silently ignored.** A report command routes every non-empty format through `emitStructured` (`root.go`), so csv/tsv/yaml render and raw/md are refused when the value has no document body; checking only for `--json` and falling through to a human printer is the anti-pattern that made fourteen commands print prose for csv and for raw. `refuseBodylessFormat` runs at the TOP of a handler, before the vault open and before any provider call: `search`/`list` would otherwise refuse only after the zero-result early return, and `suggest-links`/`suggest-target`/`polish` would fail on credentials instead of on the format, which is how the credential-free CI job went red. A JSON-event-STREAM command (`models bench`, `ai engine pull|rm`) cannot render as one document and refuses other formats by name (`refuseNonJSONStream`). `git diff`, `export-context` and `skills show` are the exceptions, and they are symmetric: their output IS a body, so raw/md/text emit it, json wraps it in a record (`{path, diff}` / `{bundle, docs, chars}` / `{slug, agent, project_path, user_path, version, content, chars}`), and csv/tsv/yaml are REFUSED (none of a diff, a context bundle or a SKILL.md is a row set). A WRITE emits a small result record naming what changed and the count where there is one (`mcp install`/`reap`, `models add|remove|enable|disable|enable-state`, `metrics clear`, `config set`, `vault create`/`set`, `skills install`/`uninstall`), built BEFORE anything prints and emitted through `emitStructured` with the human lines after it, so a format can never carry prose AND a record. In csv/tsv, composite cells are compact JSON (`output.delimitedCell`), never Go syntax, while a SCALAR payload is one PLAIN cell (the JSON-record fallback quoted it and the csv writer then escaped those quotes, so `config get ai.provider --format csv` came out as `"""bedrock"""`); a pointer cell is its VALUE (`%v` on the pointer printed a heap address) and a NIL pointer is an EMPTY cell (`<nil>` is Go syntax too, and json means `null` there; a nil MAP keeps `null`, which is JSON). **The contract is enforced by ENUMERATION, not by inspection** (`format_contract_enumeration_test.go`): the persistent flags are on the root, so every runnable command advertises them, and every one must sit in exactly one of two maps, an argv table that RUNS it or an allowlist naming why it cannot be run (PAID, NETWORK, BLOCKS, writes host state, or the stated decision that a cobra-generated `completion <shell>` script has no record shape). A new command fails the suite until it is classified, and the assertion is that STDOUT parses as JSON in full, so `jsonPortion()` is deliberately absent there. Cycle 1 converted the commands someone looked at and missed six; cycle 5 found eight more by hand, which is why the table someone must remember to extend is not the mechanism. `text` is a READING format, not a parsing one: a row list renders as `name=value` pairs per line, a single struct as `name: value` lines, a map sorted, all through the same `delimitedCell` and the json field names, with nil and `omitempty`-zero fields omitted; it used to be `%v`, which is a Go struct dump. Point anyone parsing at `tsv` or `json`. **`fieldSpecs` follows `encoding/json`'s own field rules**, with exactly one exception, named below: an anonymous struct field with no json name is FLATTENED (its index becomes a path, and a nil embedded pointer renders empty rather than panicking), a `json:"-"` field is skipped, and a same-name collision resolves json's way in `dedupeFieldSpecs` (names group by EFFECTIVE name, the shallower field wins outright, a tag breaks an equal-depth tie, and equal depth with both tagged or neither is a CONFLICT that drops the name entirely). An embedded UNEXPORTED struct is NOT an exception: reflect refuses `Interface()` on the embed itself but not on the exported fields beneath it (`Value.Field` does not propagate the embedded read-only flag), so an untagged one is flattened like any other, and a TAGGED one is assembled from those same leaves into one compact-JSON object cell, the nested object json emits. The embed itself is never passed to `Interface()`; only cell CONTENT differs there (sorted keys, since it is built as a map, and `{}` where json writes `null` for a nil embed). Skipping only unexported fields is what made `doctor --format text` and `skills doctor --format text` print a `SuiteStatus:` / `Verification:` blob keyed by a Go type name. **The one exception is an embedded type that renders ITSELF** (TextMarshaler/json.Marshaler): Go promotes its marshal method to the OUTER type, so json collapses the WHOLE record to that single value and discards every sibling field. No row-shaped format can represent that, so the row is rendered instead, and an unexported such embed is dropped outright since its value cannot be read; nothing in `internal/` has either shape. `TestFieldNames_MatchJSONThroughCollisionsAndUnexportedEmbeds` and `TestFieldNames_TaggedUnexportedEmbedIsOneObjectCell` re-read every rule off `json.Marshal` on each run, the exception included, so a change in the standard library trips the suite rather than splitting the views. `yaml` is the JSON view in YAML syntax: `writeYAML` marshals to JSON and re-reads it, so the field names, tags and `omitempty` are the json ones (yaml.v3 alone lowercases bare Go field names and honors no json tag). The PERSISTED yaml writers in ai/vault/document are separate and untouched.
