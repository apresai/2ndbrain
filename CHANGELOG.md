...
- **2ndbrain is macOS only, and the code now says so.** Both products only ever shipped for macOS, but the CLI still carried Windows branches for the Obsidian registry, the process check, the Claude Desktop config path and the engine lookup. None of it was built, none of it could be tested, and one branch was quietly wrong (the process check would have called every running process dead). They are gone, the README says the requirement up front, and a non-macOS platform now gets a clear error instead of a path that looks supported
