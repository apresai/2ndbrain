...
### Fixed
- **One unreadable note could throw away a whole re-embed.** A note whose frontmatter would not parse counted as a FAILED embedding, and `2nb index --force-reembed` treats any failure as an incomplete run, so it restored the previous embeddings and exited non-zero. On a 314-note vault that meant three rebuilds in a row that each embedded 313 documents over 70 to 90 seconds and then discarded all 313. Nothing was ever sent to the provider for the unreadable note, so it is no longer counted as an embedding failure: the rebuild finishes, keeps what it embedded, and names the notes to fix, in the `--force-reembed` summary, in `index --json` (`unparseable`), and in the MCP `kb_index` result. The whole-vault index also drops the index row of a note that has stopped parsing, the way it drops a deleted file's row, so the last readable version stops answering searches from a copy that no longer exists on disk. `2nb index --doc` on a single broken note still fails loudly, because an editor saving one note needs to hear that it did not land, and the macOS app now shows that failure beside the Index state instead of only writing it to a debug log
- **A note with an empty properties block would not parse at all.** A note that opens with `---` immediately followed by `---` has empty frontmatter, which is what Obsidian writes when you remove a note's last property, and what 2nb itself writes for an empty property set. 2nb searched past the empty block for a closing `---` and found the next horizontal rule in the body instead, then tried to read your prose as YAML. Such a note failed on every index and every embedding pass, and it is the note that caused the rebuild failure above
...
- **Notes under an Obsidian template folder are now removed from the index, not just skipped.** Marking a folder as your template folder stopped 2nb indexing new files there, but anything already indexed kept its entry: those notes went on appearing in search and in `2nb vault status` counts indefinitely, with no way to clear them short of deleting the index. They are now purged on the next index, and the run reports how many
- **A note with real properties behind a doubled `---` lost them.** 0.22.3's empty-properties fix read every note that opens with `---` immediately followed by `---` as having no properties at all, which moved the properties of a note written that way into its body. Both readings are now tried and the one that finds real properties wins
...
- **The index summary printed every unreadable note and then told you to re-run with `-v` to see them.** It now names up to five and mentions `-v` only when it actually held some back
...
- **The macOS app tells you when a note failed to index.** A note whose properties break as you save it dropped out of search silently, and stayed out until the next full index; the app wrote the reason to a debug log nobody reads. The Index section now names the note and the reason, and clears it when that note indexes cleanly. One broken note also no longer stops the rest of a batch of edits from being indexed
