...
### Fixed
- Provider-readiness failures now report **why** instead of blaming credentials for everything. The old message was `is not ready (check credentials)` for every cause, so a five-second network blip sent users off to re-authenticate; a timeout now reads `is not ready (timeout). The probe timed out. …` with the same remediation `models test` and the macOS app show for that code (the one exception is `access_denied`, below). Covers `index --force-reembed`, `ai embed-probe`, `ask`, `chat`, `polish`, `eval`, `eval answers`, `suggest-links`, `suggest-target`, and the MCP tools `kb_ask`/`kb_polish`/`kb_suggest_links`, and the message names the role ("embedding" or "generation") so it says which thing you were doing. `mcp doctor`'s AI-readiness line carries the cause too, and now also distinguishes an embedder that never registered from one whose probe failed. Providers that cannot classify their own failure (ollama, openrouter, llama-local) now say so plainly instead of pointing at credentials they do not have
