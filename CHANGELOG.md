...
### Changed
- Dependencies: AWS SDK `service/bedrock` 1.66.7 to 1.67.0, `smithy-go` 1.27.8 to 1.27.10, `modernc.org/libc` 1.75.3 to 1.75.5

### Fixed
- Provider-readiness failures now report **why**. The old messages never said: most surfaces printed a bare `provider "X" not available`, and `index --force-reembed` and `ai embed-probe` printed `is not ready (check credentials)`, which sent users off to re-authenticate after a five-second network blip. A timeout now reads `is not ready (timeout). The probe timed out. …`, carrying the same remediation `models test` and the macOS app show for that code, and the message names whether **embedding** or **generation** failed. Covers `index --force-reembed`, `ai embed-probe`, `ask`, `chat`, `polish`, `eval`, `eval answers`, `suggest-links`, `suggest-target`, and the MCP tools `kb_ask` / `kb_polish` / `kb_suggest_links`
- A provider that cannot classify its own failure (Ollama, OpenRouter, llama-local) now says the cause was not reported, then points at the right thing for each: Ollama's daemon and endpoint, the bundled engine, or `OPENROUTER_API_KEY`. Previously all three were told to check credentials, which two of them do not have
