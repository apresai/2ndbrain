...
### Changed
- `IndexGeneration` 3 to 4. **A reindex is recommended after upgrading** (`2nb index`) so notes whose dates, titles, types, statuses or tags were written unquoted carry those values in the index. No embeddings are regenerated: the date fix cannot change a note's text, so this is an index-only pass and costs nothing. `2nb vault status` and `2nb ai status` prompt for it until you run it
