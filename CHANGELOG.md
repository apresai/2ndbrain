...
### Fixed
- Every readiness surface now reports **why** a provider is not ready instead of blaming credentials for everything. The old message was `is not ready (check credentials)` for every cause, so a five-second network blip sent users off to re-authenticate; a timeout now reads `is not ready (timeout). The probe timed out. …` with the same remediation `models test` and the macOS app show for that code. Covers `index --force-reembed`, `ai embed-probe`, `ask`, `chat`, `polish`, `eval`, `eval answers`, and `suggest-links`, and the message names the role ("embedding" or "generation") so it says which thing you were doing
