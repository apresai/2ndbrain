...
### Fixed
- **Two MCP tools accepted a call missing an argument their own schema declares required, and answered it.** `kb_search` without a `query` fell through to the query-less listing path and returned EVERY document with `score: 0`, dressed as search results, so an agent that built a malformed call got a whole-vault dump that reads like ranked hits (the CLI refuses the same input outright). A blank `query` was worse: it leaked a raw `SQL logic error: fts5: syntax error near ""` to the client. `kb_update_meta` without `fields` rewrote the note with the frontmatter it already had and reported success, so an agent believed it had written metadata that never changed. All three now return a clear error. Declaring `required` in an inputSchema does not enforce anything at runtime, so every declared-required argument is now covered by a test that reads the requirement off each tool's own schema, which means a tool added later is covered without anyone remembering to add it
