...
- **A note's whole body could be destroyed by an unrelated edit.** 2nb ended a note's properties at the first line beginning `---` anywhere in the file, without checking it was the end of the file, so an ordinary markdown horizontal rule (`----`, or a longer one), a closing `---` carrying a trailing space, or a line like `---more` cut the note short: everything below it was read as if it were not there. That alone would have been a display bug, but 2nb rewrites the whole file from what it read, so the next `2nb meta --set` or `2nb tag add` on that note SAVED the truncated version and the rest of your writing was gone. A closing line must be exactly `---` now, and so must the opening one; a trailing space, tab, non-breaking space or zero-width character is still fine at either end, since none of them is visible in an editor, and 2nb treats them the same way it treats a blank line. Anything else is your text, and a note whose properties are never closed is read as having no properties rather than as having no body. A note that uses Windows line endings keeps them: an edit used to leave the properties with Unix endings and the body with Windows ones. This shipped in 0.22.3 and earlier, so if you keep a horizontal rule in a note, reindex after upgrading and check that note first
