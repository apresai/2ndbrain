...
| Documents | `create` (`--path`/`--content`/`--overwrite`/`--append`; title becomes a slug filename, stored as `title`), `read` (alias `print`), `meta` (`--get`/`--set` [array fields like tags/aliases coerced to a list]/`--remove`; aliases `frontmatter`/`fm`/`properties`; the stale positional `meta set/get/remove` form is rewritten to the flag form), `delete` (`--force` for non-interactive/agent use), `list` (alias `files`; `--total`, `--format paths\|tree`), `append`, `prepend`, `replace`, `move`, `rename` (both link-aware: rewrite `[[wikilinks]]` + `[text](path.md)`), `daily` (`path`/`read`/`append`/`prepend`), `tasks`, `task`, `tag` (`add`/`remove`, per-note frontmatter tags) |
...
**Global flags:** `--format` (json/csv/tsv/yaml/raw/md/text; listings also `paths`/`tree`), `--porcelain`, `--json`, `--csv`, `--yaml`, `--vault`, `--verbose`/`-v`, `--copy`. An explicit `--format` is either honored or refused by name, never ignored: report commands render csv/tsv/yaml and refuse `raw`/`md` when the value has no document body (`search` and `list` refuse them whether or not anything matched), the JSON-event-stream commands (`models bench`, `ai engine pull`/`rm`) refuse any non-JSON format, and `git diff` inverts it (raw/md/text emit the diff, csv/tsv/yaml are refused). In csv/tsv a composite cell (a search row's `frontmatter`, a lint finding's `candidates`) is compact JSON. Also accepts obsidian-CLI-style `key=value` args, boolean tokens (`total`/`append`/`overwrite`), and colon-commands; `file=` fuzzy-resolves by title/alias/suffix while `path=` is strict-exact. Full mapping in [docs/obsidian-cli-mapping.md](docs/obsidian-cli-mapping.md).
