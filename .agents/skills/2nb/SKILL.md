...
| `2nb eval` / `eval tune` | Retrieval scorecard (Recall@10 / R@1 / MRR over a cached QA set generated from the vault; cost-gated one-time generation) and a threshold/weight sweep that prints suggested `config set` commands (suggest-only, near-zero cost) |
