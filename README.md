...
> **Heads-up for scripters:** `2nb search --json` and `2nb ask --json` now return envelopes (`{mode, warnings, results}` / `{mode, warnings, answer, sources}`; multi-turn asks via `--history` add `rewritten_query`). If you were parsing a raw array/object, extract `.results` / `.answer`. Every key is always present and never `null`, and a search that matches nothing still returns the envelope with `results: []`, so `| jq .results` is safe on any query. `2nb list --json` and `2nb stale --json` likewise return `[]` when empty. `ask` exits non-zero when retrieval finds nothing, so check the exit code before parsing.
