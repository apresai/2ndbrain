...
| `obsidian register-types [--write] [--force]` | Declare 2nb's property types in `.obsidian/types.json` so Obsidian shows the right editor even when a property is empty. **The only Obsidian setting 2nb ever writes**, and only on this explicit command: it MERGES (a property already declared keeps its type), backs the previous file up into `.2ndbrain/recovery/obsidian/`, writes atomically, refuses a `types.json` it cannot parse, refuses while Obsidian is RUNNING with the vault open (unless `--force`; quitting Obsidian is enough, since the check asks whether the process is there rather than trusting a flag Obsidian never clears on quit), and refuses until `migrate-properties` has run. `id` is deliberately not declared; `status` is `text`, never a list type |
