...
>
> Homebrew installs shell completions automatically. For manual installs, `2nb completion install` detects your existing completion directory from your zsh config (falling back to `~/.zsh/completions/_2nb`) and adds the required `fpath` and `compinit` block to it. The config is `$ZDOTDIR/.zshrc` when `ZDOTDIR` is set, otherwise `~/.zshrc`; `--rc <path>` points it anywhere else, and `--no-rc` installs only the script and prints the two lines to add by hand. The file is copied to `<rc>.2nb-backup` before its first change. The block is placed before early-return guards and before any existing compinit/fpath setup (e.g. from gcloud or Homebrew), so completions load correctly even when other tools initialize the completion system first. Re-running is safe: the block is replaced in-place, not duplicated. If multiple `2nb` binaries are found on PATH, a warning is printed showing each binary's version and which is active.
...
- **Broken-link resolution, no dead ends** — `2nb lint` and the dashboard's Validation tab don't just flag broken `[[wikilinks]]`; every finding gets exactly one machine recommendation (`suggest-target --verdict`, prompt selected by a measured eval — see `docs/link-prompt-eval.md`): repair case/separator drift, relink to a "did you mean?" pick (source-note context + `--llm` re-rank), or **remove the dead link** when nothing matches confidently (the model's explicit decline is the removal signal). **Fix all** applies only high-confidence rewrites (drift + high); **Remove dead links** batches the removal-recommended ones behind a pre-checked confirm; everything else surfaces top 2-3 recommendations for a human pick, with create-the-note always available
...
Every catalog entry declares an `invoke_strategy` (e.g. `bedrock_converse`, `bedrock_invoke_cohere_embed`, `openrouter_chat`) so adding a new model variant doesn't require a code change — a catalog entry with the right strategy is enough. The macOS dashboard's **Models** tab (AI → Models…, Cmd+Shift+,) is the simple front door: vendor checkboxes with Check all / Uncheck all, one **Validate** button that probes what your account can actually invoke, an always-visible API-key chip, and Answers/Search pickers over the proven working set. The full catalog browser (provider cards, per-model Test / Set active / Enable / Disable / Discover) lives behind its **Full catalog** disclosure. Catalog changes written by the CLI propagate to the running GUI via FSEvents without reopening the vault.
...
| `move <src> <dst> [--dry-run] [--force]` | Move/rename a note to a new vault-relative path, rewriting every `[[wikilink]]` AND markdown-style `[text](path.md)` link across the vault that points at it (preserving heading/block/alias/embed suffixes on wikilinks, the label text + `#anchor`/`?query` suffix + `.md` extension on markdown links, and the bare-vs-path form; external-URL and anchor-only markdown links are skipped; links inside code are untouched; percent-encoded targets like `My%20Note.md`, the form Obsidian generates for paths with spaces, are matched by their decoded form and re-encoded on rewrite). `--dry-run` previews the rename, per-note rewrites, and ambiguous skips without writing; without `--force` a bare-name-ambiguous move is refused, where "name" is anything the resolver accepts (filename, path suffix, frontmatter title, alias), so two notes sharing a title block a link to that title too. The target file moves LAST for crash safety |
...
| `suggest-target <target> [--limit 6] [--source <path>] [--llm] [--verdict]` | Ranked "did you mean?" existing notes a broken `[[wikilink]]` target might have meant, ordered best-first by confidence tier then score, from three tiers: drift (the `repair-links` fuzzy index), semantic (embeddings, when configured), and BM25 keyword. With `--source`, semantic/BM25 also use surrounding note prose (context-aware), and that note is excluded from candidates. `--llm` re-ranks the grounded shortlist with the generation model when none is already high-confidence (adds a one-line `reason`, never invents paths, caps confidence at medium — the prompt and the cap were selected by a measured eval, `docs/link-prompt-eval.md`). `--verdict` wraps the output in `{recommendation, llm, candidates}`: one machine recommendation per link — relink to the top candidate (high = one-click, medium = confirm), else unlink; an explicit model decline recommends unlink even over a medium candidate. Read-only; default output stays a bare `[]`-not-null array. Pair with `relink` / `unlink` to apply |
| `relink <path> --from <target> --to <note> [--write]` | Repoint a broken link (wikilink or markdown) to a chosen existing note (preserves `#heading`/`#^block`/`\|alias`; markdown occurrences get a path-based destination that resolves); EXACT match scoped to `--from`. `--write` applies + snapshots (revert with `polish <path> --undo`) |
...
| `stale --since <days>` | Find stale documents (reads the indexed `modified` date, so run `2nb index` once after upgrading past 0.22.3) |
...
| `obsidian migrate-properties [--write]` | Rewrite quoted date values into the plain form Obsidian types as **Date and time**. Obsidian reads a quoted ISO value as Text, so every `created`/`modified` written before 0.23.0 shows as plain text with no date picker and no date sorting. Touches only `created`, `modified` and any field your `.2ndbrain/schemas.yaml` declares `date`/`datetime`; date-shaped properties you author are reported and left alone. Every other byte of a note is preserved, comments and key order included. Previews by default; `--write` snapshots each note first, so `2nb polish <path> --undo` reverses it. Idempotent, and a value that is not a parseable date is named and skipped rather than guessed at |
| `obsidian register-types [--write] [--force]` | Declare 2nb's property types in `.obsidian/types.json` so Obsidian shows the right editor even when a property is empty. **The only Obsidian setting 2nb ever writes**, and only on this explicit command: it MERGES (a property already declared keeps its type), backs the previous file up into `.2ndbrain/recovery/obsidian/`, writes atomically, refuses a `types.json` it cannot parse, refuses while Obsidian is RUNNING with the vault open (unless `--force`). On macOS and Linux quitting Obsidian is enough, since the check asks whether the process is actually there rather than trusting a flag Obsidian never clears on quit; where that cannot be determined it refuses and says so rather than asserting Obsidian is open, and refuses until `migrate-properties` has run. Every property 2nb writes is declared, `id` included; `status` is `text`, never a list type |
...
| `config bedrock` | Show or set machine-local Bedrock region, token, additional verify regions, and key precedence (`~/.config/2nb/bedrock.json`; `--set --regions us-west-2,us-east-2` / `--clear-regions`; `--set --prefer-stored-token` makes the saved key win over `AWS_BEARER_TOKEN_BEDROCK` for all 2nb use while the env var keeps serving other tools). Does not need a vault. `--json` redacts the token, and both JSON and human output warn when the env var overrides a different saved key (informational instead once prefer-stored is on) |
...
All commands support `--json`, `--yaml`, `--csv`, `--tsv` for machine-readable output, plus `--format raw`/`md` to emit a document body (or any `Serialize()`-able value) verbatim, `--format text` for plain text, and (on listings) `--format paths`/`tree` and `--total`. A value with no document body refuses `raw`/`md` and points at `--json`; `search` and `list` refuse them up front, so the refusal does not depend on whether the query matched anything. `git diff`, `export-context` and `skills show` are the inverse: their output is a body, so `raw`/`md`/`text` emit it, `--json` wraps it in a record, and `csv`/`tsv`/`yaml` are refused. `--format text` renders a list of rows as one line of `name=value` pairs per row and a single record as `name: value` lines, for reading rather than parsing; the field names are `--json`'s, including the fields of an embedded record, which `text` and `csv` promote to the top level the way `--json` does. An empty listing (`list`, `stale`, `tags`, `aliases`, `orphans`, `deadends`, `unresolved`, `backlinks`, `links`) is `[]` under `--json` and `--yaml`, and the header row alone under `csv`/`tsv`. `search` and `ask` are the exception: they carry an envelope object rather than a bare array, so a zero-hit `search` emits its full `{mode, warnings, results}` envelope under `--json` and nothing at all on stdout under `yaml`/`csv`/`tsv`/`text`, and `ask` exits non-zero when retrieval finds nothing. In `csv`/`tsv`, a cell holding a map, slice or struct (a search row's `frontmatter`, a lint finding's `candidates`) is compact JSON with sorted keys rather than Go syntax, and an unset pointer field is an EMPTY cell rather than the Go token `<nil>`. A command whose whole output is a single scalar (`config get`, `meta --get`) emits one plain cell under `csv`/`tsv` rather than a quoted JSON string. A command that WRITES (`models add`/`remove`/`enable`/`disable`/`enable-state`, `metrics clear`, `config set`, `vault create`/`set`, `skills install`/`uninstall`, `mcp install`/`uninstall`/`reap`) emits a small record naming what changed, so a machine format on any of them is parseable rather than empty or prose. Commands whose machine output is a stream of JSON events (`models bench`, `ai engine pull`, `ai engine rm`) accept only `--json` and refuse other formats by name. `--copy` also writes a command's rendered output to the clipboard (macOS `pbcopy`): `read`/`print`, `meta --get`, and `daily` copy in their default output; any command run with a machine format copies that output.
...
- **Similarity threshold**: hybrid search drops vector hits whose cosine similarity is below the active threshold so barely-related neighbors stop padding result lists. Resolution order: explicit vault config (`2nb config set ai.similarity_threshold 0.25`) > a user calibration saved by `2nb models calibrate --save` (or `2nb models add --similarity-threshold`, the only two writers, both of which stamp the row `threshold_source: user`) > per-model recommendation from the builtin catalog > global default `0.20`. A user-catalog value that is unstamped and identical to the builtin recommendation is treated as a copy, not a calibration, so a catalog contaminated by an older `models bench` run heals itself on the next read. Builtin recommendations: Nova-2 `0.25` (measured; queries embed with Nova's asymmetric `GENERIC_RETRIEVAL` purpose, which collapses the cosine scale; see [docs/search-tuning.md](docs/search-tuning.md)), Nemotron `0.60`, nomic-embed-text/Titan-v2/Cohere-embed `0.50`, mxbai/snowflake/bge-m3 `0.55`, all-minilm `0.35` (all estimated from each model's training objective, so run `2nb models calibrate` to tune for your vault). Override per-query with `2nb search "foo" --threshold 0.35`. `2nb ai status` shows the active value and which tier supplied it.
- **Calibration**: `2nb models calibrate` samples random chunk pairs from your vault, reports the noise-floor cosine distribution (p50/p90/p95/p99), and recommends a threshold. Add `--save` to persist it to the per-vault user catalog (or `--save --scope global` for all vaults); it is stamped as yours, and no probe, benchmark or promotion will overwrite or invent one.
...
- **AI card**: provider and models (AWS Bedrock with Claude Haiku 4.5 + Amazon Nova-2 by default) with a readiness dot, an always-visible link to the AI settings (where the API key lives), a Test button, and a Reset-to-recommended-defaults button that appears only when the config has drifted
