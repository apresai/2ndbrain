...
| `config bedrock` | Show or set machine-local Bedrock region, token, and additional verify regions (`~/.config/2nb/bedrock.json`; `--set --regions us-west-2,us-east-2` / `--clear-regions`). Does not need a vault. `--json` redacts the token, and both JSON and human output warn when `AWS_BEARER_TOKEN_BEDROCK` in your environment overrides a different saved key |
