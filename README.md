...
- **Broken-link resolution, no dead ends** — `2nb lint` and the dashboard's Validation tab don't just flag broken `[[wikilinks]]`; every finding is resolvable: repair case/separator drift, pick a "did you mean?" suggestion (`suggest-target` with source-note context and optional `--llm` re-rank → `relink`), create the missing note, or `unlink` it. **Fix all** applies only high-confidence rewrites (drift + high); everything else surfaces top 2-3 recommendations for a human pick
...
| `suggest-target <target> [--limit 6] [--source <path>] [--llm]` | Ranked "did you mean?" existing notes a broken `[[wikilink]]` target might have meant, from three tiers best-first: drift (the `repair-links` fuzzy index), semantic (embeddings, when configured), and BM25 keyword. With `--source`, semantic/BM25 also use surrounding note prose (context-aware), and that note is excluded from candidates. `--llm` re-ranks the grounded shortlist with the generation model when none is already high-confidence (adds a one-line `reason`, never invents paths, caps confidence at medium). Read-only; emits `[]` not null. Pair with `relink` to apply a pick |
