...
**CLI-UB-007**: Write commands (those that create or modify notes — e.g. `create`, `append`, `prepend`, `replace`, `meta --set`, `move`, `index`, the `kb_*` write tools, and the MCP server itself) shall resolve the target vault more strictly than read commands: from `--vault`/`2NB_VAULT` if given, otherwise the vault Obsidian has open (or, when Obsidian is closed, its most-recently-opened vault). The current directory shall never be an implicit write target except when it is itself the vault root; a current directory that resolves a vault only by walking up to a parent shall be refused rather than written to, and the system shall never auto-create a `.2ndbrain/` sidecar implicitly on a write. If `--vault`/`2NB_VAULT` names a vault Obsidian does not know, the write shall be refused unless the user passes `--unconfigured` (or sets `2NB_UNCONFIGURED=1`), which permits the write and warns that the note will not appear in Obsidian or the 2nb index. This guard prevents an agent launched from an unrelated directory from silently splitting a vault.
