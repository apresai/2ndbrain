...
describe('commandTimeoutMs', () => {
	...
	it('bounds ask above THREE sequential Go ai.MantleWorstCase transport legs', () => {
		// A chat/--history ask runs up to four sequential worst-case legs
		...
		expect(
			...
			`ask timeout must exceed 4 x Go ai.MantleWorstCase (4 x ${mantleWorstCaseMs}ms = ${4 * mantleWorstCaseMs}ms; each leg is 3 attempts x 240s MantleAttemptTimeout + 3s backoff, cli/internal/ai/timeouts.go): a chat/--history ask can run four sequential worst-case legs (history-condense generation, query embed, the zero-result fallback re-embed in ask.go's askOnce, answer generation), and an outer bound at or below their sum fires first and fails a working-but-cold model, the exact inversion PR1 removed from the CLI`
		).toBeGreaterThan(4 * mantleWorstCaseMs);
		expect(commandTimeoutMs('ask')).toBe(2_940_000);
	});
