...
**What is not covered.** Only the Bedrock providers implement `AvailabilityReporter`; ollama, openrouter, and llama-local answer a bare bool, so their failures carry no code and every surface says the provider "did not report a cause" rather than guessing one. That wording is deliberate: those providers have no credentials to check, and the message this replaced said "check credentials". Giving them real codes is a per-provider probe change (each already discards the error it would need) and is the natural follow-up.
