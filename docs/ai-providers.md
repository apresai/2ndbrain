...
**Why it matters:** `2nb index --force-reembed` refuses to run against an unready provider, and the message it printed was `is not ready (check credentials)` for **every** cause. A five-second network blip therefore accused the user's credentials. The message now names the cause (via `ai.NotReadyMessage` -> `ai.ReadinessRemediation`), so a timeout reads `is not ready (timeout). The probe timed out. …` and only a genuine auth failure mentions credentials. A provider that cannot classify itself says so and points at where to look, rather than guessing (see **What is not covered** below).
...
**One vocabulary, three renderers, sized to their surface.** All live in `cli/internal/ai/not_ready.go`. `NotReadyMessage(kind, provider, code)` produces the full sentence used by the CLI paths (`ask`, `chat`, `polish`, `index --force-reembed`, `ai embed-probe`, `eval`, `eval answers`, `suggest-links`, `suggest-target`) and the MCP twins (`kb_ask`, `kb_polish`, `kb_suggest_links`). `NotReadySummary(code)` is the short clause behind `ai status`'s per-provider `reason` and `mcp doctor`'s detail. The `VectorCompat` degradation banner formats its own line and carries the CODE ONLY, deliberately: it prints on every degraded query, so the remediation would make it several hundred characters. The split is deliberate: **`internal/cli` owns the carrier** (`providerReadiness`, which holds a resolved verdict and travels between surfaces), **`internal/ai` owns the words**.

**What is not covered.** `AvailabilityReporter` is implemented by the classic Bedrock embedder, generator, and reranker only. ollama, openrouter, llama-local (and `BedrockMantleGenerator`, which reports available whenever it holds a token) answer a bare bool, so their failures carry no code. Those surfaces say the provider "did not report a cause" and then name where to look, per provider (`ai.UnclassifiedNextStep`): Ollama's daemon and endpoint, `OPENROUTER_API_KEY`, the bundled engine.

That phrasing is deliberate in both directions. The wording it replaced said "check credentials" for every provider, which is wrong for ollama and llama-local, which have none. But it was NOT wrong for openrouter, whose probe sends a bearer token and can fail on a rejected key, so the credential hint survives there rather than being dropped everywhere in the name of caution. Giving these providers real codes is a per-provider probe change (each already discards the error it would need) and is the natural follow-up.
...
**One `access_denied` override.** The readiness probe is a control-plane listing, so a denial means the IAM principal cannot call `bedrock:ListFoundationModels`. `RemediationFor` answers for `models test`, which invokes a model, and so reads `access_denied` as model entitlement and points at the Bedrock console's "Model access" page. That page cannot grant an API permission, so `ai.ReadinessRemediation` substitutes text naming the permission. Every other code means the same thing on both paths, and a test in `internal/ai` guards the divergence now that both functions are neighbours.
...
**A FAILED fetch is cached too, and that is load-bearing.** Only successes were stored, so a fetch that could not finish was retried in full by the next caller, and the one after that. The `AmazonBedrock` offer file is 16MB against that 15s deadline, so on any link slower than roughly 1MB/s to `pricing.us-east-1.amazonaws.com` it CANNOT complete: `models list` paid about 30s on every invocation rather than once, and one test binary paid it twelve times and blew Go's 10m package timeout. The 30s is TWO SEQUENTIAL LEGS, not two files: `EnrichModelPricing` fetches OpenRouter first and Bedrock after it, while the two Bedrock offer files inside `loadBedrockPricing` go out on their own goroutines and so cost one 15s deadline between them, not two. `loadCachedHTTPBody` now stamps a failure and suppresses another attempt for `pricingFetchCooldown` (10 minutes), still preferring a stale cache over a wait. The fix lives in that one function rather than in `loadBedrockPricing` and `loadOpenRouterPricing`, which had the identical `if pricing.ready` shape and would have drifted.

The cooldown is recorded in TWO places because neither covers the other: an on-disk stamp in the pricing cache directory carries a failure to the NEXT process, which is what the CLI needs since every invocation is a fresh one, and an in-memory map covers repeated calls within one process, which is what the MCP server and a test binary need. **Both are keyed by the offer URL, never by the cache entry's filename.** The two Bedrock offer URLs carry no region while their cache filenames do (`bedrock-us-east-1.json` and `bedrock-us-west-2.json` are the same bytes), so a path-keyed stamp would be keyed on a dimension the URL does not have, and a machine with a second configured region would re-pay the full stall against a URL the first had already proved was failing. The later of the two deadlines wins, so an old stamp cannot cut an in-process cooldown short and a fresh one still cools a process that has not failed itself; a success clears both, so a recovered network is picked up without user action. `make test` redirects `XDG_CACHE_HOME`, so a failed fetch during the test suite cannot leave a stamp the developer's own `models list` would then read.
