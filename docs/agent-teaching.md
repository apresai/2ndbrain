...
| `"semantic search disabled: provider X not ready (CODE) — falling back to keyword search"` | The configured provider failed its readiness probe. `CODE` is one of `timeout`, `throttled`, `provider_unreachable`, `bad_credentials`, `access_denied`, `invalid_request`, `unknown`. The same token appears in `ai status --json` as a trailing `(code)` inside `providers[].reason` — there is no separate code field | No immediate fix. BM25 still runs. `timeout`/`throttled`/`provider_unreachable` usually clear on their own; `bad_credentials`/`access_denied` need `2nb ai status`. |
