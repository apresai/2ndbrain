...
4. **A search row carries the note's frontmatter, and an enumerated row carries its first chunk.** Every row has `doc_id`, `path`, `title`, `chunk_id`, `heading_path`, `content`, `score`, `type`, `status`, and `frontmatter` (the parsed frontmatter map, omitted when empty). That map holds RESOLVED values, so a DATE renders as the instant it parses to rather than as the note wrote it: a file saying `created: 2026-07-14` comes back `"2026-07-14T00:00:00Z"` in this field. It is decoded from the `documents.frontmatter` JSON column written at index time, so no per-key original text reaches a search row; when you need what the note actually says, ask `meta --get <key>` (or MCP `kb_read`), which reads the note. `frontmatter` is additive as of 0.22.1 and appears on CLI `search` rows and MCP `kb_search` rows alike, so an agent can filter or cite on metadata without a second `kb_read`. The **enumerate-by-filter** form (a blank query plus a filter, `2nb search "type:adr"`) returns the same shape: before 0.22.1 those rows carried empty chunk fields and the note's frontmatter JSON in `content`, so enumerating gave you a JSON blob where you expected prose. A note with no body has no chunks and still enumerates, with `chunk_id`, `heading_path` and `content` empty.
...
| `"semantic search disabled: provider X not ready (CODE) — falling back to keyword search"` | The configured provider failed its readiness probe. `CODE` is one of `timeout`, `throttled`, `provider_unreachable`, `bad_credentials`, `access_denied`, `not_found`, `invalid_request`, `unknown` (treat an unrecognized code as `unknown`). The same token appears in `ai status --json` as a trailing `(code)` inside `providers[].reason` when the provider can classify itself; there is no separate code field, and rows for providers that cannot classify carry a plain-language reason with no code. Do not blindly take the last parenthesis: a bedrock row that never reached a probe reads `region not set (ai.bedrock.region)` or `bedrock not registered (check ai.provider setup)`. Match against the known codes and ignore anything else | No immediate fix. BM25 still runs. `timeout`/`throttled`/`provider_unreachable` usually clear on their own; `bad_credentials`/`access_denied` need `2nb ai status`. |
