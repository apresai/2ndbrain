...
| `schema_version` | Single-row schema version. Schema v2 adds the token columns via an idempotent `ALTER TABLE ADD COLUMN` migration, so an existing `metrics.db` keeps its history (old rows default to 0) rather than being dropped |
