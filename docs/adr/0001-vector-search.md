...
- **Latency regression.** It is a cost-optimized, infrequent-query tier (~100ms warm, sub-second cold; ~90% cheaper storage; up to 2B vectors/index), and slower than our local sqlite-vec/brute-force at current scale (≈4ms at 1K docs, ≈34ms at 10K). AWS itself routes low-latency work off S3 Vectors to OpenSearch.
