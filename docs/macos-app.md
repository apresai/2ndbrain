...
An `FSEventsWatcher` on the vault keeps the index fresh: notes edited in Obsidian are incrementally re-indexed and re-embedded a moment after they settle (a debounced `2nb index --doc` via `scheduleExternalReindex`, skipping the app's own writes; a note that fails to index is reported as `AppState.lastIndexingProblem` and shown beside the Index state, where it used to be swallowed into a debug log, and the batch drain catches per path so one broken note no longer aborts the rest of a sync). A full index SETS it too, and that is why `startIndex` runs `2nb index --json` (never `--porcelain`, which would silence the stderr progress lines the sheet is built on) and decides from `AppState.FullIndexSummary` rather than from a regex over the old prose summary: exit 0 stopped meaning "nothing was wrong" once an unparseable note became non-fatal and had its row dropped, so a clean rebuild can hide a note that just vanished from search. `applyFullIndexOutcome` reports the first entry of `unparseable`/`unreadable` with a count of the rest, clears only when both lists are empty, and on an envelope that does not decode keeps whatever was already reported rather than dismissing it. The report is otherwise cleared when THAT note indexes cleanly (`clearIndexingProblem(for:)`, the single-note watcher's own path-scoped clear) and on `openVault(at:)`, since the path belongs to the vault being left behind. `openVault(at:)` also cancels the external-reindex debounce and drops `pendingExternalPaths` (and the per-path `reindexTasks`), beside the `fileWatcher?.stop()` that queued them: that queue is AppState-wide rather than vault-scoped, so a change queued against the old vault would otherwise drain 1.5s later against the new one, where `relativePath(for:)` falls back to `lastPathComponent` and could reindex a same-named note in the wrong vault. A call already executing stays pinned to the vault it started with and finishes there; cancellation is not cooperative inside `runCLI`, so the guarantee covers what is QUEUED, and on bind a one-shot incremental `2nb index` (`syncOnBindIfStale`, gated on an on-disk-vs-indexed count delta) catches up notes added or removed while the app was closed, so embeddings stay current without a manual Sync.
...
**The tab's `--discover` reads the cache, not the network.** `modelsListArgs` is unchanged and passes no new flag: since 0.22.3 `models list --discover` is served from the CLI's 24h discovery cache, so a reload no longer re-walks both Bedrock planes across three regions (about 11 seconds, measured) for a catalog that changes when AWS ships a model. A vendor the user unchecked is skipped by the walk entirely, so the Models refresh also stops emitting a warning per disabled provider. The Validate pane's **Refresh** button is the deliberate re-walk and already passed `models discover --refresh`.
