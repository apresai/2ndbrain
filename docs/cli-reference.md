...
- `candidates`: the repair index's matches when `fix` is `ambiguous`, each usable as a `relink --to` (relink derives the resolving markdown form from the picked note, so a title candidate is safe for markdown occurrences too), so a UI can offer the pick list without a suggest-target round-trip.
