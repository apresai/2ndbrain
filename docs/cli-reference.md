...
- `--get <key>` reads one field (exit `ExitNotFound` if absent). The default output renders a scalar through `document.ScalarText`, so an unquoted frontmatter date reads as RFC3339 and agrees with what `--json` and `--format text` already emitted; plain `%v` printed Go's own `2020-01-01 00:00:00 +0000 UTC` for that value.
...
`--dry-run` previews the rename, the per-note rewrites, and the ambiguous links it would skip, without writing anything. Without `--force`, a move is refused when a bare `[[name]]` link is ambiguous (the name matches more than one note); `--force` rewrites only the unambiguous path-qualified links and leaves the bare ones. **`name` means whatever the resolver accepts:** a filename, a path suffix, a frontmatter title, or an alias. Before 0.22.3 the guard checked the filename only, so two notes sharing a TITLE (`2nb create` slugs the filename and keeps the title verbatim, so two notes called "Dup" are `one/dup.md` and `two/dup.md`) left a `[[Dup]]` link invisible to it: that link resolves to nothing, so it is not a backlink and its text never names either file, and the move completed with `skipped_ambiguous` empty and `refused` false, after which the link silently pointed at the note that was NOT moved. The guard asks `store.Resolver` now, so "ambiguous" here means what it means in `lint`, `repair-links` and `suggest-target`. The target file is moved LAST, after referencing notes are rewritten, so a crash leaves links pointing at the still-present old name. JSON result: `{moved, rewritten, skipped_ambiguous, failed, dry_run, refused, move_failed}`. `refused` is true when the ambiguity guard stopped a non-force move before any write, and the human summary then says so and uses "Would move" phrasing; it used to print "Moved:" in the past tense on a move that never happened. `move_failed` is true when the final rename (or its directory creation) failed; the summary then says "Move FAILED" and reports how many referencing notes had already been rewritten to the destination, since `rewritten` is the plan and a note whose write failed appears in `failed` too.
...
Its output IS a document body, so it takes the same shape as `git diff`: the default form, `raw`, `md` and `text` emit the markdown bundle; `--json` wraps it in a `{bundle, docs, chars}` record; and `csv`, `tsv` and `yaml` are refused by name, before the vault is opened, since a bundle is not a row set. `docs` and the bundle's own `N documents included.` header both count the notes that actually made it in, so a note whose file cannot be parsed (it is skipped with a warning on stderr) is counted by neither. The header used to state the number the QUERY matched, so a vault with one unreadable note announced one more document than the bundle held. Before 0.22.3 the command never consulted `--format` at all: `--json`, `--csv` and `--yaml` all printed the raw markdown and exited 0, so `2nb export-context --json | jq .` failed to parse on a command reporting success. With no matching documents, `--json` still emits a record (`docs: 0`, empty bundle) and the other forms print the "No documents match the filters." hint on stderr.
...
- The ROUTE column names the Bedrock endpoint a row invokes: plane and region (`classic us-east-1`), the plane alone for a row with no region pin, and `-` where a row carries no plane, which covers every non-Bedrock provider and any catalog row written before routes existed. Rows are routes, so a Bedrock model served on both planes or in several regions occupies one line per endpoint, differing in this column and carrying its own STATE. Compose the qualified `id@plane/region` form from it for `config set ai.<slot>_model` and `models test`.
...
**Format handling.** `config get` prints its bare scalar with no format and under `--porcelain`, which is the scripting contract `$(2nb config get ai.provider)` depends on; an explicit `--format` renders the value instead. `getConfigValue` returns the value already formatted as text, so json emits a quoted string (`"bedrock"`, `"1024"`) rather than guessing a type back out of it. `config get` used to print the bare scalar for EVERY format, which is not JSON. `config set` emits `{key, value}` under an explicit format, with the value read back from the SAVED config rather than echoed from the argument (a model-slot write goes through `applyModelSlotRoute` and can store a different string than was typed); it used to write nothing at all to stdout.
...
`--format raw` (and `md`) emits a value's `Serialize()` output (or the raw string/bytes) with no JSON wrapping, for piping a document body verbatim; a value with no body (a search result list, a report) is refused with a pointer at `--json`, rather than printed as a Go struct dump. `tsv` is tab-separated CSV. An unknown `--format` value is refused and the valid set is listed; it used to fall through to JSON silently.

`text` renders a document body verbatim (so `read --format text` and `git diff --format text` print the body) and every other value as named lines: a list of rows as one line per row of `name=value` pairs, a single struct as one `name: value` per line, a map sorted by key. Names are the json ones, so the three views agree about what a field is called; a pointer renders as its value, a composite as compact JSON, a date as RFC3339, and a nil or `omitempty`-zero field is left out. That agreement now holds through an EMBEDDED record too: an anonymous struct field with no json name has its own fields promoted to the top level, exactly as `--json` promotes them, and a field tagged `json:"-"` appears in none of the three views. Before 0.22.3 every one of those shapes went through Go's `%v`, so `text` emitted a debug dump of the struct: `2nb list --format text` printed `{cb9316a1-… resources/x.md Title note draft 2026-…}` (positional, no names), `2nb folders --format text` printed `{resources 5}`, and `2nb config show --format text` printed a heap address for its nested config pointer. Text and csv also skipped only unexported fields, so `2nb doctor --format text` printed one `SuiteStatus: {"latest":…}` blob and `2nb skills doctor --format text` a `Verification: {"slug":…}` blob, keyed by a Go type name the json view never emits, on the two commands a human is likeliest to point `--format text` at. **`text` is for reading and `tsv` is for cutting:** a value containing a space is not quoted and nothing is escaped, so parse `tsv` or `json` instead.

**Every report command honors every format.** Fourteen commands used to check only for `--json` and fall through to their human printer otherwise, so `--csv`, `--tsv` and `--yaml` were silently ignored and `--format raw`/`md` printed prose instead of refusing: `mcp status`, `mcp configured`, `git status`, `git activity`, `git show`, `index --doc`, `polish`, `polish --undo`, `relink`, `unlink`, `repair-links`, `suggest-links`, `suggest-target`, and `config bedrock`. All of them route every explicit format through the shared writer now. A payload that is a list of rows renders in `csv`/`tsv` as a header plus one line per row; any other shape (a single status struct, a map) renders as one JSON-encoded record, which is what the writer has always done for such a shape, so `csv` is honored rather than dropped but is not a table. `search` and `list` refuse `raw`/`md` before opening the vault, so the refusal depends on the command rather than on whether the query matched anything, and a hybrid search never pays for a query embedding it cannot render. `git diff` and `export-context` are the mirror image, because their output IS a body: `raw`, `md` and `text` emit it, `--json` wraps it in a record (`{path, diff}` / `{bundle, docs, chars}`), and `csv`, `tsv` and `yaml` are refused by name, since neither a diff nor a bundle is a row set. Commands whose machine output is a STREAM of JSON events (`models bench`, `ai engine pull`, `ai engine rm`) cannot render as one document and refuse any non-JSON explicit format by name.
...
**An empty result is still a JSON document.** Every command that can legitimately return zero rows emits a parseable payload on stdout under `--json`: `search` returns its full `{mode, warnings, results}` envelope with `results: []`, and the bare-array listings (`list`, `stale`, `tags`, `aliases`, `orphans`, `deadends`, `unresolved`, `backlinks`, `links`, plus `related`'s nested `edges`) return `[]`, never `null` and never nothing. That holds for the bare form and `--porcelain` as well as `--json`, since the listings render JSON by default, and for `lint`'s nested `issues`. That matters most for `search`: `warnings` is where a vault reports that its semantic channel degraded to keyword-only, so a query matching nothing is exactly when a caller needs the envelope. An empty LISTING returns the same `[]` under `--yaml` from 0.22.3: it renders by marshalling to JSON first, so a nil slice arrived as `null` and printed `null`, disagreeing with its own json view and with `tasks --yaml`, whose slice merely happens to be built non-nil at its construction site. In `csv` and `tsv` a zero-row listing is the HEADER ROW with no data under it, which is a csv document a consumer can bind columns from; before 0.22.3 a zero-row slice missed the struct-slice branch and fell through to the single-JSON-record fallback, so `orphans --format csv` printed the literal `null` and `tasks --format csv` printed `[]` into a delimited stream, the exact corruption the delimited formats exist to avoid. **`search` and `ask` are the exception, because they are not listings.** Their payload is an ENVELOPE OBJECT rather than a bare array, so `[]` is not its empty form in any format, and only `--json` carries the envelope at all. A zero-hit `search` therefore emits its full `{mode, warnings, results}` document under `--json` and NOTHING on stdout under `yaml`, `csv`, `tsv` and `text` (the "No results found" hint goes to stderr and the exit code stays 0): it returns before the writer, and a literal `[]` dropped into a delimited stream is the corruption those formats exist to avoid. `raw`/`md` are refused up front, as they are on any row set. `ask` exits NON-ZERO with an error when retrieval finds nothing rather than answering emptily, so check the exit code before parsing. Human output is unaffected, with each command's empty-state hint on stderr (`list` prints its hint there and nothing on stdout, on an empty vault as on a filter miss).
