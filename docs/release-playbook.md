...
2. `make release`: updates `CHANGELOG.md`, commits, tags `v<VERSION>`, pushes the tag. `scripts/update-changelog.sh` builds that entry from human-authored content wherever it exists, and only falls back to a commit summarizer when there is none. Hand-curated `[Unreleased]` entries are folded in as written; an explicit changes file (the optional second argument) is additive to them, both being human-authored. The summarizer runs ONLY when neither exists: it and curated entries describe the same commits, so letting both contribute produces one entry that says everything twice under duplicate headings, which is what shipped in 0.20.0. Whichever path supplies the content, `[Unreleased]` is reset to its placeholder and the entry is echoed back as written. Guarded by `tests/changelog-script-test.sh`, which runs in `make test` (so every `make release-all`) and `make test-release-script`.
