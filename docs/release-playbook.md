...
2. `make release`: updates `CHANGELOG.md`, commits, tags `v<VERSION>`, pushes the tag. The changelog entry's content is chosen by `scripts/update-changelog.sh` with a strict precedence: an explicit changes file if the caller passed one, else the hand-curated `[Unreleased]` entries (authoritative, folded in verbatim), else a commit summarizer as the last resort. Curated entries and the summarizer never both run: they describe the same commits, so combining them produces one entry that says everything twice under duplicate headings (that shipped in 0.20.0). Whichever path supplies the content, `[Unreleased]` is reset to its placeholder. Guarded by `tests/changelog-script-test.sh` (`make test-release-script`).
